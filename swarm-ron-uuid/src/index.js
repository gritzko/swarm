...
const RON: { [string]: RegExp } = require("swarm-ron-grammar");
...
class UUID {
  ...
  static CODES: any; // FIXME
  static Vector: any;
  static Iterator: any;
  static TIME_CONST: { [string]: 1 };
  ...
  static fromString(
    ...
    if (!m || m.index !== off) return UUID.ERROR;
    if (offset === undefined && m[0] !== string) return UUID.ERROR;
...
*/
   ...
   * @returns {UUID}
   */
  static as(smth: UUID | { toString: () => string }): UUID {
...
codes.fill(-1);
for (let i = 0; i < B64.length; i++) codes[B64.charCodeAt(i)] = i;
UUID.CODES = codes;

class Vector {
  static Iterator: any; // FIXME
   ...
   */
  constructor(uuids: string, default_uuid?: UUID) {
...
class Iterator {
   ...
   * @param default_uuid {UUID=}
   */
  constructor(body?: { toString: () => string }, default_uuid?: UUID) {
    /** type {String} */
  ...
  nextUUID() {
    if (this.offset === this.body.length) {
      ...
      else this.offset = UUID.RE.lastIndex;
