...
SecondPreciseClock.prototype.issueTimestamp = function time () {
    ...
    if (seq>=(1<<12)) { // FIXME
        throw new Error('max event freq is 4000Hz');
    }

    var baseTimeSeq = base64.int2base(res, 5);
    if (seq>0) {
        baseTimeSeq += base64.int2base(seq, this.seqWidth);
    }
...
SecondPreciseClock.MAX_SEQ = base64.int2base(3999,2);
