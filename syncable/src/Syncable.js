...
function Syncable(id_or_null, host) {
    ...
    if (Host.multihost) {
        if (arguments[1]) { // TODO prototype._host
            if (arguments[1].constructor!==Host) {
                throw new Error('the host is not a Host');
            }
    ...
    var doubl = host.linkSyncable(this);
    return doubl;
...
Syncable.prototype.getInnerState = function () {
    return this.host().getInnerState(this);
};
...
Syncable.prototype.toString = function () {
    var keys = Object.keys(this).filter(function(k){
        return Syncable.reFieldName.test(k);
    });
...
Syncable.Ref.prototype.fill = function (host) {
    ...
    return this._target = this.host().get(this.ref);
    // TODO .4 #0 null-like obj
};
