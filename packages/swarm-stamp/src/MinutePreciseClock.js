...
var MinutePreciseClock = function (processId, timeOffsetMs) {
    if (!LamportTimestamp.reTok.test(processId)) {
...
MinutePreciseClock.prototype.issueTimestamp = function () {
    ...
    var baseTime = base64.int2base(time, 4), baseSeq;
    if (seq<64) {
        baseSeq = base64.int2base(seq, 1);
    } else {
        baseSeq = base64.int2base(seq, 3);
    }
...
MinutePreciseClock.prototype.parseTimestamp = function parse (ts) {
    ...
    return {
        time: base64.base2int(time),
        seq: base64.base2int(seq)
    };
};
