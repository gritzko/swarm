...
exports = ( function () {
    ...
    */
        ...
        deliver: function (action,spec,value,lstn) {
            ...
            if (childId) { // FIXME  .on .off fits the childid def
                ...
                switch (action){
                    case 'on':
                    case 'reOn':
                        ret = this.on(spec,val,lstn);
                        break;
                    case 'off':
                    case 'reOff':
                        ret = this.off(spec,val,lstn);
                        break;
                    case 'set':
                    default:
    ...
    EventRelay.extend(Swarm, {
        ...
        // THINK:
        //  * swarm is a sink for pipes,
        //  * manages uplink/routing logic
        //  * reentry point for local filtering
        //  isn't it a mess?!!
        //  NO
        //  * each op sinks to the target
        //  * then, gets relayed (except to the source)
        //  * remote subscriptions are kept here (?)
        on: function () {
    ...
    EventRelay.extend(Field, {
        ...
        set: function () {
            // no direct invocation Spec.normalizeSig3(this,arguments);
            var spec=arguments[0], value=arguments[1], lstn=arguments[2];
            //if (this.validate && !this.validate(spec,value,lstn))
            // TODO test   throw new Error('invalid value: '+value);
            this.value = value;
