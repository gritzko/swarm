...
exports = ( function () {
    ...
    Swarm.inherit = function(parent,protoProps, staticProps) {
        //var parent = this;
        if (typeof(parent)!=='function') throw new Error('no parent given');
        var child;
        ...
        if (staticProps)
            Swarm.extend(child, parent, staticProps);

        var Surrogate = function(){ this.constructor = child; };
        Surrogate.prototype = parent.prototype;
        child.prototype = new Surrogate;

        if (protoProps) Swarm.extend(child.prototype, protoProps);
    ...
    var EventRelayProto = {
        ...
        on: function (filter,base,newln) {
            ...
            if (this.fwdOn) {
                var fwd = this.fwdOn(filter,base,newln);
                if (fwd)
                    return fwd.on(filter,base,newln);
            }
            filter.within(this.spec());
            // We expect filtered listeners to be an exception
        ...
        set: function (spec,val,src) {
            ...
            spec = new Spec(spec.toString(),'.');
            /*if (typeof(spec)==='object' && spec.constructor!==Spec && !value) {
                value = spec;  // set({key:value})
                spec = null;
            }
            spec = Spec.as(spec,this.spec(),'.'); // mandatory scope
            // generate vid
            if (!spec.author)
                spec.author = Swarm.author;
            if (!spec.time)
            ...
            /*if (!spec.member) {
                ...
                updates[spec.member] = value;*

            this.apply(spec,val,src);
    ...
    Swarm.extend(Swarm,EventRelayProto);*/
    ...
    Swarm.extend(EventRelay.prototype, {
        init: function (id) {
            this._id = id;
            this._lstn = [];
        },
        spec: function () {
            var ret = new Spec();
            ret[this.myKey] = this._id;
            return ret;
        },
        fwd: function (args) {
            var args = Swarm.normalize3Sig(this.spec(),arguments);
            var childId = args.spec[this.fwdKey];
            if (!childId) return null;
            var child = this.children[childId];
            if (!child) {
                if (this.create)
                    child = this.children[childId] = this.create(childId);
                else
        ...
        on: function () {
            var args = Swarm.normalize3Sig(this.spec(),arguments);
            if (this.acl && !this.acl(args))
                throw new Error('access denied: '+args.spec);
            var fwd = this.fwd && this.fwd(args);
            if (fwd)
                return fwd.on(args);
    ...
    Swarm.extend(Swarm, EventRelay.prototype, {
        fwdKey: 'type',
        spec: function () {
            return new Spec();
        },
    ...
    Type = Swarm.inherit(EventRelay, {
        create: function typeCreate (spec) {
            ...
            // FIXME this is an abstract 'type', should produce relay stubs
            var obj = new this(spec.id);
        }
    });

    // var TodoItem = Model.inherit({});
    // var item = TodoItem.create();
    //      item
    //      var BoolField = Field.inherit({});
    //

    var Model = Swarm.inherit(EventRelay, {
        myKey: 'id',
        fwdKey: 'member',
        init: function (id) { // optional: values???
            this._id = id;
            this._fields = {};
            var fp = this.constructor.fieldPrototypes;
            for(var name in fp)
                this.fields[name] = new fp[name](name,fp._fieldDefaults[name]);
        },
    ...
    var Field = Swarm.inherit(EventRelay, {
        init: function (id) {
    ...
    Model.addTrackedField = function (proto, name, defaultValue) {
        ...
        // TODO here we may note that defaultValue is an object
        // having acl(), validate(), default etc...
        if (typeof(defaultValue)!=='object')
            fieldPrototypes[name] = Field.extend({
                defaultValue: defaultValue,
                _id: name
            });
        else
            fieldPrototypes[name] = Field.extend(defaultValue);
    };
    ...
    // BIG INTERESTING TODO:  fitting `extend` to the cascade

    // TODO
    //  * remoteMethods
    Model.extend = function (fn,mixin) {
        var proto = fn.prototype;
        var fname = fn.name; // IE? || options.name;
        Swarm.types[fname] = fn;
        fn.id = fname;
        fn.objects = fn.children = {};
        fn.fieldProtos = {};
        Swarm.extend(fn,Model.prototype);
        Swarm.extend(proto,Type.prototype);
        for(var member in mixin)
            if (typeof(mixin[member])==='function')
                Model.addLoggedMethod(proto,member,mixin[member]);
            else
                Model.addTrackedField(proto,member,mixin[member]);
    };
