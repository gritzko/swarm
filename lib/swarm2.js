    ...
    */
    ...
    EventRelay.extend(EventRelay,{ // :)
        ...
        deliver: function (spec,value,lstn) {
            ...
            if (childId) { // FIXME  .on .off fits the childid def
                ...
                if (!child) {
                    ...
                    child = this._children[childId] = new this._defaultChild(childId,undefined,this);
                }
        ...
        off: function () {
            Spec.normalizeSig3(this,arguments);
            var spec=arguments[0], value=arguments[1], lstn=arguments[2];
            spec.action = spec.action||'off'; // FIXME reOff
    ...
    EventRelay.extend( Model, {
        ...
        set: function () { // TODO maybe route field sets here as well
            ...
            if (!spec.version && !lstn) { // newly introduced op
                ...
                return Swarm.root.deliver(spec,value,this);
            }
        ...
        // Returns the current version of the object's data in a rather
