    ...
    */
    ...
    EventRelay.extend(EventRelay,{ // :)
        ...
        deliver: function (spec,value,lstn) {
            ...
            } else if (Spec.isMap(value)) {
                for(var key in value) {
                    child = this.child(key.substr(1)); // :(
                    if (!child)
                        throw new Error('invalid bundle');
                    child.deliver(new Spec(spec+key), value[key], lstn); // FIXME potentially incorrect spec
                }
            }else{
                switch (spec.action){
                    case 'on':
            ...
            if (child && this._isCumulative && (spec.action||'set')==='set')
                this.emit(spec,value,lstn);
            return ret;
        },
        ...
        on: function () {
            this.normalizeSig3(arguments);
            var spec=arguments[0], value=arguments[1], repl=arguments[2];
            spec.action = spec.action||'on';
            if (this.child(spec)) // user called us directly, maybe?
                return this.deliver(spec,value,repl);
            // synchronize, i.e. send a diff back to the other replica
            if (this.sync && typeof(repl.set)==='function')
                this.sync(spec,value,repl);
            // add a listener
            this._lstn || (this._lstn=[]);
            if (this._lstn.indexOf(repl)===-1)
                this._lstn.push(repl);
            if (spec.action==='reOn') {
                // synchronize children to the uplink; 
                if (this._children)
                    this.recursiveOn(repl);
            } else {
                if (typeof(repl.reOn)==='function')
                    repl.reOn(this.scope(),this.version(),this); 
            }
        },
        
        recursiveOn: function (repl) {
            for(var id in this._children) {// send back a ton of on()
                var child = this._children[id];
                repl.on(child.scope(),child.version(),child);
            }
        },
        
        version: function () {
            if (!this._isCumulative || !this._children)
                return this._version;
            var versionMap = new Spec.VersionVector(this._version);
            for(var id in this._children)
                versionMap.put(this._children[id].version());
            return versionMap.toString();
        },
        
        sync: function (spec,version,replica) {
            var diff = typeof(this.diff)==='function' && this.diff(version);
            diff && replica.set(this.scope(),diff,this);
            if (this._isCumulative && this._children)
                for(var id in this._children)
                    this._children.sync(spec,version,replica);
        },
        ...
        off: function () {
            ...
            var ll=this._lstn;
            if (!ll) return;
            var i = ll.indexOf(repl);
            if (i===-1) // maybe a wrapper
                for (i=0;i<ll.length && ll[i].set!==repl.set; i++);
            if (i===ll.length)
                throw new Error('listener unknown: '+spec);
            ll.splice(i,1);
            if (spec.action==='reOff') {
                if (this._children)
                    for(var id in this._children) {// send back a ton of off()
                        var child = this._children[id];
                        repl.off(child.scope(),child.version(),child);
                    }
            } else {
                if (typeof(repl.reOff)==='function')
                    repl.reOff(this.scope(),this.version(),this); 
            }
        },
    ...
    EventRelay.extend(Swarm, {
        ...
        set: function () {
            ...
            var spec=arguments[0], value=arguments[1], lstn=arguments[2];
            spec.action = 'set';
            if (this.child(spec)) return this.deliver(spec,value,lstn);
            /*if (!spec.version)
                spec.version = Spec.newVersion();
            return this.deliver(spec,value,lstn);*/
            this.timeOffset = value - new Date().getTime();
        },
        version: function() {
            return Spec.newVersion();
        },
        sync: function(spec,value,peer) {
            var time = Spec.base2int(Spec.bare(value));
            var myTime = Spec.base2int(Spec.bare(Spec.newVersion()));
            if (Math.abs(time-myTime)>2) {
                if (this.author==='root' && peer.author!=='root') {// TODO
                    //peer.set('',Spec.newVersion(),this);
                } else if (this.author!=='root' && peer.author==='root') {
                    Spec.offset = time - myTime;
                } else if (this.author==='root' && peer.author==='root') {
                    throw new Error('clock mismatch: '+Spec.newVersion()+' vs '+value);
                }
            }
        },
    ...
    EventRelay.extend( Model, {
        ...
        set: function () { // TODO maybe route field sets here as well
            ...
            if (!spec.version) { // newly introduced op
                ...
                // (if it throws, let it throw now, before it is propagated)
                return this.root().deliver(spec,value,this);
            }
            
            if (this.child(spec))
                this.deliver(spec,value,lstn);
                
            if (!value && typeof(value)!=='object')
                throw new Error('the value must be an object');
                
            this._version = spec.version; // remember the version
            this._value = value; // store the snapshot
            
            // clear old vals
            for(var key in this)
                if (key.charAt(0)!=='_' && typeof(this[key])!=='function')
                    this[key] = undefined;
            // set new user-visible values
            // later on, these values may be modified by logged methods
            // and direct assignments; on save() the new snapshot will
            // be saved
            for(var key in value)
                if (key.charAt(0)!=='_')
                    if (!this[key])
        ...
        noRecursiveOn: true,
        /* On one side, universal 'on' impl is good
           on the other side, transmitting {prop:version} maps may be more bw than values
           Issues:
            * method/property emit() -> parent listeners
            * on(vers_vec)
                * diff(): combine props/meth tails
                * version() -> vector
            v may subscribe to a property/method
            v supress recursive subscription
        
            After all, it is only an optimization
        
            Q: invoke child on OR parent.emit() ?
        ...
        */
        ...
        sync: function (spec,base,repl) {
            var diff = this.diff(base);
            if (diff)
                repl.set(this.scope(),diff,this);
        }
    });
    ...
    EventRelay.extend(Field, {
        ...
        init: function (id, value, parent) {
            ...
            if (value) {
                this._value = value;
                this._version = Spec.newVersion();
            } else {
                this._value = this.constructor.defaultValue;
                this._version = '';
            }
        },
        set: function () {
    ...
    Model.addProperty = function (name, value, type) {
        ...
        proto[name] = function (val) {
            if (val)
                ...
                return this._children[name]._value;
        };
    ...
    EventRelay.extend(LoggedMethod, {
        ...
        diff: function () {
            // NOTE that diff is not a bundle but a { version : arguments } map
            // because we need to apply those in order
            // /Type#id.method { v1: [], v2: [] }
        }
    });
    ...
    function Transport () { // TODO bundle specvals
    }
