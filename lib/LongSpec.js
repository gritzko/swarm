...
LongSpec.Iterator.prototype.encode = function encode (de) {
    ...
    var enBody=undefined, enExt=undefined;
    if (pm) {
    ...
    if ( body.length<=4 ||          // TODO make it a switch
        ...
        if (maySeq && prevBody.length===2) {
            var seqBody = LongSpec.int2uni(int-this.prevCollapsed-1);
        }
    } else if (body.length===7) { // 3-symbol base-2^15
        var int1 = Spec.base2int(body.substr(0,5));
        var int2 = Spec.base2int(body.substr(5,2));
        var uni1 = LongSpec.int2uni(int1);
        var uni2 = LongSpec.int2uni(int2).charAt(1);
        enBody = uni1 + uni2;
        if (maySeq && prevBody.length===3) {
            var seqBody = uni1 + LongSpec.int2uni(int2-this.prevCollapsed-1).charAt(1);
        }
    } else if (body.length===10) { // 4-symbol 60-bit long number
        var int1 = Spec.base2int(body.substr(0,5));
        var int2 = Spec.base2int(body.substr(5,5));
        var uni1 = LongSpec.int2uni(int1);
        var uni2 = LongSpec.int2uni(int2);
        enBody = uni1 + uni2;
        if (maySeq && prevBody.length===4) {
            var seqBody = uni1+LongSpec.int2uni(int2-this.prevCollapsed-1);
        }
    } else { // verbatim
        enBody = body;
        seqBody = enBody;
...
LongSpec.Iterator.prototype.decode = function decode () {
    ...
    switch (body.length) {
        case 1:
            ...
            break;
        case 2:
            var int = LongSpec.uni2int(body);
            var base = Spec.int2base(int,5);
            de += base;
            break;
        case 3:
            var int1 = LongSpec.uni2int(body.substr(0,2));
            var int2 = LongSpec.uni2int('0'+body.charAt(2));
            var base1 = Spec.int2base(int1,5);
            var base2 = Spec.int2base(int2,2);
            de += base1 + base2;
            break;
        case 4:
            var int1 = LongSpec.uni2int(body.substr(0,2));
            var int2 = LongSpec.uni2int(body.substr(2,2));
            var base1 = Spec.int2base(int1,5);
            var base2 = Spec.int2base(int2,5);
            de += base1 + base2;
            break;
        default:
            de += body;
...
LongSpec.Iterator.prototype.skip = function ( count ) {
    ...
    if ( leftInChunk <= count ) { // skip chunks
        ...
        for(var c=this.chunk+1;    // how many extra chunks to skip
            left>chunks[c] && c<chunks.length;
            c++, left-=chunks[c]);
        this.skip2chunk(c);
    }
...
LongSpec.Iterator.prototype.skip2chunk = function ( chunk ) {
    ...
    if (chunk>0) { // (1) chunks must not be empty; (2) a chunk starts with a full token
        ...
        for(var j=0; Spec.quants.indexOf(prev.charAt(prev.length-1-j))!==-1; j++);
        this.prevCollapsed = j;
        for(var k=0; Spec.quants.indexOf(prev.charAt(prev.length-1-j-k))===-1; k++);
