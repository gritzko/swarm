...
Storage.prototype.deliver = function (spec, value, source) {
    ...
    console.log('@' + this.id, spec.toString(), value, '('+(source&&source.id)+')');
    if (op.spec.op()==='bundle') {
        var ops = op.unbundle();
        if (ops.length===0) return; // empty bundle is OK
        op = ops.shift();
...
Storage.prototype.process = function (op) {
    ...
    function loadTail () {
        ...
        self.db.createReadStream({
            gte: self.id + request.prefix + request.need_mark, // start at the mark (inclusive)
            lt: self.id + request.prefix + request.mark // don't read the next object's ops
        })
...
Request.prototype.patch = function () {
    ...
    var tail = null, m;
    if (!recent) {
        tail = null; // I don't have state => no response
    } else if (base==='') {
...
Request.prototype.preon = function (source) {
    ...
    if (!base) {
        ...
        if (later===this.deriveVersionVector()) return later;
        base = this.vv;
    }
    this.response.push({
