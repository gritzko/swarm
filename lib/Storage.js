...
/**
Storage is responsible for keeping operation logs and state snapshots and doing all the log-related work. As Host and Syncables can only see the log on op-by-op basis, all incoming ops are first filtered by a Storage (to detect replays).

A storage only talks to its Host (which does multiplexing to local Syncables and remote subscribers). Storage responds to every incoming subscription (.on) with an object state or a log tail or both or neither. That response is always a .bundle. The response is followed by a reciprocal subscription (.reon). The .deliver(spec,value) interface is the same all along the chain (Syncable<->Host<->Storage).

A Storage relies on its underlying storage engine to actually persist and retrieve the data. We imply an ordered key-value storage engine. All the versioning related machinery is too much nitty-gritty to reliably replicate it for every backend, so Storage does it all.

Operations, state snapshots and other records are stored under string keys that form an alphanumeric total order:
...
Storage.prototype.ssnSuffix = 'store';


Storage.prototype.deliver = function (spec, value, source) {
    var id = spec.id();
    console.log('storage got ' + spec);
    // ops may be processed asynchronously in parallel, except that
    // we must ensure same-id ops are sequential (to avoid data races)
    var op = { spec:spec, value:value, source:source };
...
Storage.prototype.process = function (op) {
    ...
    function loadTail () {
        ...
        if (request.need_mark===null) throw new Error('no mark provided');
        self.db.createReadStream({
            gte: request.prefix + request.need_mark, // start at the mark (inclusive)
            lt: request.prefix + request.mark // don't read the next object's ops
        })
        .on('data', function (data){
            var key = data.key.substr(request.prefix.length);
            request.ops.push( { 
                spec: key, 
                value: data.value 
            } );
    ...
    function send (err) {
        ...
        request.response.forEach(function(o) {
            self.host.deliver(o.spec, o.value, self);
        });
        next();
    }
    ...
    function error (err) {
        ...
        self.host.deliver( op.spec.set('.error'), err );
        next();
    }
...
Request.prototype.dispatch = function dispatch (request, callback) {
    ...
    switch (this.op) {
        ...
        case 'on':      return self.on();
        case 'reon':    return self.reon(); 
        case 'preon':   return self.preon(); // TODO plan B  src===null
        case 'off':    
        case 'reoff':   return self.off(); 
        case 'state':   return self.state();
        case 'bundle':  return self.bundle();
        default:        return self.anyop(); 
    }
};
...
Request.prototype.patch = function () {
    ...
    if (base==='') {
        ...
        if (this.mark>recent) {
            this.retryOnLoad(recent);
            return;
        }
        if (!this.areBackrefsClosed()) return;
            var state;
            // FIXME
        tail = this.ops.filter( function (op) {
            var version = new Spec(op.spec).version();
    ...
    if (tail) {
        var bundle = tail.map( function(op) {
            return '\t' + this.prefix+op.spec + '\t' + op.value + '\n';
        });
        this.response.push({
            spec: this.spec.set('.bundle'),
            value: bundle.join('')
        });
    }
    return done;
...
Request.prototype.versionVector = function () {
    var recent = this.meta['.recent_state'];
    if (!recent) {
    ...
    this.ops.forEach(function(op){
        ...
        vv.add(spec.version());
        if (spec.op()==='br') vv.add(op.value);
    });
    this.vv = vv.toString();
    return done;
};

Request.prototype.preon = function (source) {
    source = source || this.spec.source();
    var base = this.meta['.bm&'+source]; // try find a bookmark
    if (!base) {
        ...
        base = this.versionVector();
        if (this.need_mark) return later;
    }
    this.response.push({
        spec: this.spec.set('.on'),
        value: base
    });
    return done;
};
...
/** Accept a subscription for an object: open it, send back a patch
  ...
  */
Request.prototype.on = function (source) {
    var ok = this.patch();
    ...
    if (!this.meta['.base_state']) { 
        ...
        base = '';
    } else if (Spec.reTokExt.test(this.value)) {
        ...
        // Unless we have nothing. FIXME 
        base = this.meta['.tip'];
    } else {
        // In other cases, we send the version vector as a last resort.
        if (this.versionVector()===later) return later;
        base = this.vv;
    }
    this.response.push({
        spec: this.spec.set('.reon'),
        value: base
    });
    return done;
};

Request.prototype.reon = function (source) {
    // FIXME either this.value or .ebm
    return this.patch();
};
...
Request.prototype.anyop = function storageOnOp () {
    ...
    if (tip===this.version) { // fast-track echo
        this.write['.ebm&'+this.source] = tip; // not spec.source()!!!
    } else if (tip>this.version) { // arrival order <> total order mismatch
        // have to check the db: whether it is an echo or a reorder
        if (!this.markLoaded(version)) return later;
        var tiptok = new Spec(tip,'!').token('!');
        var out_of_order = this.ops.some(function(op){
            var v = Spec.token(op.spec,'!');
            return v.ext===tiptok.ext && v.bare>tiptok.bare;
        });
        if (out_of_order) {
            // There is a younger operation from the same source=>
            // causal order is violated, can't write this.
            this.error = 'operation is out of order';
            return;
        } 
        ...
        this.response.push({ spec: this.spec, value: this.value });
    ...
    if (this.options.bookmarking) {
        ...
        this.write['.bm&'+this.source] = this.version;
    }
    return done;
};
