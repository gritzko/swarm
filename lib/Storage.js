...
Storage.prototype.process = function (op) {
    ...
    function error (err) {
        ...
        self.host.deliver( new Op(op.spec.set('.error'), err, self.id) );
        next();
    }
...
Request.prototype.respond = function (opname, value) {
    var op = opname ? 
        new Op(this.spec.set(opname), value, this.myid) :
        new Op(this.spec, this.value, this.myid);
...
Request.prototype.patch = function () {
    ...
    if (tail) {
        ...
        this.respond('.bundle', bundle.join(''));
    }
    return done;
};
...
Request.prototype.preon = function () {
    //var ok = this.patch();
    //if (ok===later) { return later; }
    var target = this.value;
    var base = this.meta['.bm&'+this.target]; // try find a bookmark
    if (!base) {
...
Request.prototype.on = function () {
    ...
    this.respond('.reon', base);
    return done;
};

Request.prototype.reon = function () {
