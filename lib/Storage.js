...
Storage.prototype.on = function (spec,base,src) {
    ...
    this.readState(ti,function(err,s){
        state = s || {_version:'!0'};
        if (tail!==undefined) respond();
    });

    this.readOps(ti,function(err,t){
        tail = t || null;
        if (state!==undefined) respond();
    });
};
...
Storage.prototype.readState = function (ti, callback) {
    var state = JSON.parse (JSON.stringify (this.states[ti]||null) );
    callback(state); // TODO async
    //this.async ? setTimeout(reply,1) : reply();
};

Storage.prototype.readOps = function (ti, callback) {
    var tail = JSON.parse (JSON.stringify (this.tails[ti]||null) );
    callback(tail);
};
