...
A storage only talks to its Host (which does multiplexing to local Syncables and remote subscribers). Storage responds to every incoming subscription (.on) with an object state or a log tail or both or neither. That response is always a .diff. The response is followed by a reciprocal subscription (.reon). The .deliver(spec,value) interface is the same all along the chain (Syncable<->Host<->Storage).
