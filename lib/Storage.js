...
Storage.prototype.deliver = function (spec, value, src) {
    ...
    case 'state':
        ret = this.init(spec, value, src); break;
    case 'bundle':
        var ti = spec.filter('/#'); // FIXME reuse Syncable.bundle
        var specs = [], s;
        for(s in value) {  specs.push(s);  }
        specs.sort();
        while (s=specs.pop()) {
            ret = this.anyOp( ti.add(s), value[s], src);
        }
        break;
    default:
...
Storage.prototype.on = function storageOn (spec, base, src) {
    ...
    function sendResponse() {
        var bundle = {};
        if (!state) {
            if (self.isRoot && base==='' && spec.version()!==spec.id()) {
                // make 0 state for a global object TODO move to Host
                state = {_version: '!0'};
            }
        }
        if (state) {
            var state_spec = spec.tok('!') + '.state';
            bundle[state_spec] = state;
        }
        if (tail) {
            for (var s in tail) {
                bundle[s] = tail[s];
        ...
        if (state) {
            src.deliver(tiv.add('.bundle'), bundle, self);
            src.deliver(tiv.add('.reon'), Syncable.stateVersionVector(state), self); // TODO and the tail
        } else {
            src.deliver(tiv.add('.reon'), '', self); // state unknown
        }
    }
...
Storage.prototype.readOps = function (ti, callback) {
    ...
    for (var vm in tailSerialized) try {
        var json = tailSerialized[vm];
        if (json) {
            tail[vm] = JSON.parse(json);
            ops++;
        }
    } catch (ex) {
        console.error("JSON parse error: "+tailSerialized[vm], ex);
    }
    callback(null, ops ? tail : null);
};
