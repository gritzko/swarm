...
module.exports = Syncable.extend(Model, {
    ...
    normalizeOpFilter: function (filter) {
        ...
        if (typeof(filter) === 'string' &&
            ...
            res = Syncable._pt.normalizeOpFilter('.set');
            var onlySetOp = res.fn;

            res.filter += ':' + filter;
            res.fn = function onlySpecifiedField(spec, val, src) {
                return onlySetOp(spec, val, src) &&
                        (fieldName in val);
            };
        } else {
            res = Syncable._pt.normalizeOpFilter(filter);
        }
        return res;
    },
