...
module.exports = Syncable.extend(Model, {
    ...
    /**
     * @param {string|Spec} filter
     * @param {function|{deliver:function}|WrappedListener|Syncable} listener
     * @returns {{filterSpec: Spec, listener: WrappedListener}}
     * @override Syncable#wrapListener()
     */
    wrapListener: function (filter, listener) {
        //  support the model.on('field', callback) pattern
        if (typeof(filter) === 'string' &&
                (filter in this.constructor.defaults)) {

            var fieldName = filter;
            var filterFn = function setSpecifiedField(spec, val) {
                return fieldName in val;
            };
            listener = new WrappedListener(this, listener, ':' + fieldName, filterFn);
            filter = '.set';
        }
        return Syncable._pt.wrapListener(filter, listener);
    },
