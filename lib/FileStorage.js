 ...
 *   state:  state=> save state; log=> append
 *   op:     append
...
function FileStorage(dir) {
    ...
    if (!fs.existsSync(dir + '/_log')) {
        fs.mkdirSync(dir + '/_log');
    }
...
FileStorage.prototype.time = Host._pt.time;

FileStorage.prototype.version = Host._pt.version;
...
FileStorage.prototype.appendToLog = function appendToLog(ti, verop2val) {
    ...
    for (var verop in verop2val) {
        tail[verop] = verop2val[verop];
    }
    ...
    if (this.logSize > this.MAX_LOG_SIZE) {
        this.rotateLog();
    }
...
FileStorage.prototype.pullState = function pullState(ti) {
    ...
    this._host.deliver(ti.add(this.time(), '!').add('.on'), '.state!0', this);
};

FileStorage.prototype.state = function fsPatch(spec, state) {
    var ti = spec.filter('/#'),
        self = this;
    if (!state._version) { // no full state, just the tail
        return this.appendToLog(ti, state._tail);
    }
    // in the [>on <state1 <reon >state2] handshake pattern, we
    // are currently at the state2 stage, so the state in this
    // state also includes the tail which was sent in state1
...
FileStorage.prototype.on = function (spec, base, replica) {
    ...
    fs.readFile(statefn, function onRead(err, data) {
        ...
        replica.deliver(tiv.add('.state'), state, self);
        replica.deliver(tiv.add('.reon'), Syncable.stateVersionVector(state), self);
    });
    ...
    for (var i = 0; i < logs.length; i++) {
        ...
        for (var j = 0; j < arr.length; j++) {
            ...
            for (var tidoid in block) {
                ...
                for (var vidop in ops) {
                    tail[vidop] = ops[vidop];
                }
            }
            this.dirtyQueue.push(this.logCount);
        }
