...
Syncable.extend(Syncable, {  // :P
     ...
     */
    deliver: function (op, delay_emit) {
        if (op.constructor!==Op) {
            throw new Error("update sig");
        }
        var spec = op.spec, value = op.value, source = op.source;
        spec = Spec.as(spec);
        ...
        try { // FIXME .4 try-wrap impl only, emit has its own wraps
            ...
            if (this._ops[call]) {  // FIXME name=>impl table
                ...
                op_ret =
                this._ops[call].call(this, spec, value);
                ...
                // invoke the implementation
                op_ret =
                this._neutrals[call].call(this, spec, value);
    ...
    neutrals: {
        ...
        error: function (spec, val, repl) {
            console.error('something failed:', spec.toString(), val, '@', (repl && repl._id));
        },

        /** Note that events are emitted
         *  *after* the complete diff is processed. */
        diff: function (spec, value, source) {
            var self=this;
            var op = new Op(spec, value, source);
            var ops = op.unbundle();
            ops.forEach(function(op) {
                self.deliver(op, true);
            });
            this.emit4();
        },

        on: function (spec, value, source) {
            if (value==='') {
