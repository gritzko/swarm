...
Syncable.extend(Syncable, {  // :P
    ...
    deliver: function (spec, value, lstn) {
        ...
        try {
            var opName = spec.op();
            if (opName === 'init') { // "init" operation is a special case
                var knownVersion = this._version;
                // invoke the implementation
                this._ops.init.call(this, spec, value, lstn); // NOTE: no return value
                // emit "init" to listeners only when version has been changed
                if (this._version !== knownVersion || knownVersion === '!0') {
                    value = this.diff(knownVersion);
                    this.emit(spec, value, lstn);
                }
            } else if (this._ops[opName]) { // FIXME name=>impl table
                if (this.isReplay(spec)) { // it happens
                    console.warn('replay', spec);
                    return;
                }
                // invoke the implementation
                this._ops[opName].call(this, spec, value, lstn); // NOTE: no return value
                ...
                this.emit(spec, value, lstn);
            } else if (this._neutrals[opName]) {
                // invoke the implementation
                this._neutrals[opName].call(this, spec, value, lstn);
    ...
    normalizeOpFilter: function (filter) {
        var spec, fn;
        if (filter) {
            spec = new Spec(filter, '.');
            filter = spec.op();
        } else {
            spec = '';
            filter = '';
    ...
    addListener: function (listenerToAdd, isUplink) {
        if (env.trace) {
            env.log(this, '.addListener', (isUplink ? 'up' : ''), listenerToAdd.toString());
        }
    ...
    removeListener: function (listenerToRemove) {
        ...
        if (env.trace) {
            env.log(this, '.removeListener', listenerToRemove.toString(), removed ? 'OK' : 'NotFound');
        }
        return removed;
    },
    ...
    neutrals: {
        ...
        on: function (spec, filter, repl) {   // WELL  on() is not an op, right?
            ...
            var wrap = this.normalizeOpFilter(filter);
            var filterSpec = wrap.spec;
            ...
            if (!this._version && filterSpec) { // TODO solidify
                listener = new WrappedListener({
                    ...
                    listener: repl,
                    filter: wrap.filter,
                    filterFn: function waitForState(spec) {
                        return spec.op() === 'init';
                    },
                    notify: function deliverDeferred() {
        ...
        off: function (spec, filter, repl) {
            this.removeListener(new WrappedListener({
                ctx: this,
                listener: repl,
                filter: this.normalizeOpFilter(filter).filter
            }));
        },
        ...
        reoff: function (spec, filter, repl) {
            if (this._id) {
                var removed = this.removeListener(new WrappedListener({
                    ctx: this,
                    listener: repl,
                    filter: this.normalizeOpFilter(filter).filter
                }));
    ...
    checkUplink: function () {
        ...
        subscribeTo.forEach(function subscribeToUplink(up) {
            ...
            var listener = new WrappedListener({
                ctx: self,
                listener: up,
                filterFn: function waitReon(spec) {
                    return spec.op() === 'reon' && spec.version() === onSpecVersion;
                },
                notify: function pendingToNormal(spec, val) {
                    self.removeListener(listener);
                    self.addListener(new WrappedListener({
                        ctx: self,
                        listener: up,
                        filterFn: self.normalizeOpFilter('').fn
                    }), true);
                }
            });
    ...
    /**
     * Call to be notified once the object replica got the state
     * @param {function} cb callback
     */
    onStateReady: function (cb) {
        if (this.hasState()) {
            cb();
        } else {
            this.once('init', cb);
    ...
    once: function (filter, cb) {
        var self = this;
        var onceListener = new WrappedListener({
            ctx: self,
            listener: cb,
            filter: this.normalizeOpFilter(filter).filter,
            notify: function notifyOnce(spec, val, src) {
                self.removeListener(onceListener);
                onceListener.defaultNotify(spec, val, src);
            }
        });
