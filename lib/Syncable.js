...
function Syncable() {
    ...
    this._id = spec.id();
    var doubl = this._host.logics.linkSyncable(spec, this);
...
Syncable.extend = function (fn, own) {
    ...
    var SyncProto = function () {
        ...
        var syncProto = this; // TODO .4 compact, move out of here?
    ...
    /*function wrapCall(name) { // FIXME .4 kill this? ops are not API methods
        ...
        return function wrapper() {
            ...
            var op = new Op(spec, args, '0');
            this.deliver(op) &&
                this._host.deliver(op);
        };
    }*/
...
Syncable.extend(Syncable, {  // :P
    ...
    deliver: function (op) {
        if (op.constructor!==Op) {
            throw new Error("update sig");
        }
        var spec = op.spec, value = op.value;
        spec = Spec.as(spec);
        var error;
        var op_ret;
        ...
        if (error = this.validate(spec, value)) {
            throw new Error('invalid op: ' + error);
        }
        if (!this.acl(spec, value)) {
            throw new Error('access violation: '+spec);
        }
        ...
        if (this._ops[call]) {  // FIXME name=>impl table
            if (this.isReplay(spec)) { // it happens
                console.warn('replay', spec.toString(), '@'+this._host.id);
                return;
            }
            if (!this._version && spec.op() !== 'state') {
                throw new Error('op applied to a stateless object');
            }
            // invoke the implementation
            op_ret =
            this._ops[call].call(this, op);

            var vv = new Spec.Map(this._version);
            vv.add(spec.filter('!'));
            this._version = vv.toString();

            this.callReactions(spec, value);

        } else if (this._neutrals[call]) {
            // invoke the implementation
            op_ret =
            this._neutrals[call].call(this, op);

        } else {
            this.unimplemented(spec, value);
        }
    ...
    //neutrals: {
         ...
         *  *after* the complete diff is processed. */
        diff: function (op) {
            var self=this;
            var ev = this._events;
            if (ev) {
        ...
        on: function (op) {
            // ? move state push here maybe TODO
        },
    ...
    on4: function (filter, callback, context) {
        ...
        if (!this._events) {
            this._events = {listeners:[],queued:null};
        }
    ...
    emit4: function (event) {
        var ev = this._events;
        if (event && ev && ev.queued) {
            ev.queued.push(event);
        } else if (!event && ev && ev.queued) {
            var queue = ev.queued;
            ev.queued = null;
            while (queue.length) {
                this.emit4(queue.shift());
            }
        } else if (event && ev && ev.listeners) {
            event.target = this;
            ev.listeners.forEach(function(l){
                if (l.filter) {
