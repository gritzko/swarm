...
Syncable.extend(Syncable, {  // :P
    ...
    neutrals: {
        ...
        on: function (spec, filter, repl) {   // WELL  on() is not an op, right?
            ...
            if (!this._version /*&& !this.isRooted()*/) {
                this._lstn.push({
                    _op: 'reon',
                    _src: repl,
                    deliver: function () {
    ...
    // Whether some uplink is listening to us, i.e. whether we are
    // already a part of the change propagation spanning tree
    // (which is actually a DAG, not a tree, but well...)
    isRooted: function () {
        var ln = this._lstn;
        for(var i=0; ln && i<ln.length && ln[i]!==','; i++) {
            if (ln[i]._id) { // an actual uplink
                return true;
            }
        }
        return false;
