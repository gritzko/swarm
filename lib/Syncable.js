...
Syncable.extend = function (fn, own) {
    ...
    var SyncProto = function () {
        ...
        for (name in own) {
            if (Syncable.reMethodName.test(name) && own[name].constructor === Function) {
                var cons = own[name].constructor;
                if (cons === Function) { // non-op method
                    this[name] = own[name]; // these must change state ONLY by invoking ops
                } else if (con===String || con===Number) { // some static constant, OK
                    this[name] = own[name];
                } else if (name in {ops:1,neutrals:1,remotes:1}) { // see above
                } else {
                    console.warn('invalid member:',name,own[name].constructor);
                }
            } else {
        ...
        on: function (spec, filter, repl) {   // WELL  on() is not an op, right?
            ...
            if (!this._version && !self.isUplinked()) {
                this._lstn.push({
                    ...
                    _src: repl,
                    deliver: function () {
                        if (!self._version && !self.isUplinked()) {
                            return;
                        } // wait
