...
Syncable.extend(Syncable, {  // :P
    ...
    deliver: function (spec, value, lstn) {
        ...
        try {
            ...
            if (this._ops[call]) {  // FIXME name=>impl table
                ...
                if (spec.op() !== 'init') {
                    console.log("oploogd",this._id,this._host._id,spec.toString());
    ...
    apply: function (values) {
        // TODO .4  (1) rename (2) convert types
    ...
    off4: function(filter,callback,context) {
        ...
        filter = filter || null;
        context = context || null;
        this.events.listeners = this.events.listeners.filter (
            function (l) {
                return  l.callback!==callback;
            }
        );
        if (this.events.listeners.length===0) {
            this.events = null;
        }
    },
