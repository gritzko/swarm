...
function Syncable() {
    ...
    //this.reset();
    this.deliver(this.spec().add('!0.state'),this.zeroState()); // FIXME default vals
...
Syncable.extend(Syncable, {  // :P
    ...
    deliver: function (spec, value, lstn, delay_emit) {
        ...
        try {
            ...
            if (this._ops[call]) {  // FIXME name=>impl table
                ...
                this.emit(spec, value, lstn); // FIXME .4 reon lstnrs
            } else {
    ...
    /**Produces a bundle containing tail of the op log, everything that has
     * changed since the base version.
     * In some cases, the size of a distilled log is small enough to
     * use it for state transfer (i.e. no snapshots needed).
     */
    diff: function (base) { // TODO .4  move this to Storage
        if (!this._version) { return undefined; }
        this.distillLog(); // TODO optimize?
        var patch={}, count=0;
        var map = new Spec.Map(base || '');
        for (var spec_vo in this._oplog) {
            if (!map.covers(new Spec(spec_vo).version())) {
                patch[spec_vo] = this._oplog[spec_vo];
                count++;
            }
        }
        return count ? patch : null;
    },
    ...
    zeroState: function () { // FIXME .4 static
        var ret = {_version:'!0'};
        ...
        for (var name in defs) {
            ...
            switch (def.type) {
                case String:
                case Number:
                    ret[name] = def.value; // immutables
                    break;
                default:
                    ret[name] = def.value===undefined ?
    ...
    neutrals: {
        ...
        on: function (spec, filter, repl) {
            ...
            if (filter) {
                ...
                if (filter_by_op === 'state') {
                    //var bundle = baseVersion ? this.diff(baseVersion) : {};
                    repl.deliver(spec.set('.state'), this.toPojo(true), this);
                    return; // FIXME .once ?
                }
                if (filter_by_op) {
                    repl = {
        ...
        reon: function (spec, base, repl) {
            if (base) {  // a diff is requested
                var diff = this.diff(new Spec(base).filter('!'));
                if (diff) {
                    repl.deliver(spec.set('.bundle'), diff, this);
                }
            } else {
                var state = this.toPojo(true);
                repl.deliver(spec.set('.state'), state, this);
            }
        },
        ...
        bundle: function (spec, ops, source) {
            var ti = spec.filter('/#'), state_vo, vos = [];
            for(var vo in ops) {
                if (vo.indexOf('.state')!==-1) {
                    state_vo = vo;
                } else {
                    vos.push(vo);
                }
            }
            vos = vos.sort().reverse();
            // .state is processed the first, irrespectively of the timestamp
            // (it has a timestamp of the handshake, most likely the youngest)
            if (state_vo) {
                vos.push(state_vo);
            }
            while (vo=vos.pop()) {
                this.deliver(ti.add(vo), ops[vo], source, true);
            }
            this.emit4();
        },
    ...
    ops: {
       ...
       /** Syncable CmRDT objects use state machine replication. The only
        ...
        state: function (spec, state, src) {
            ...
            state._oplog && (this._oplog = state._oplog); // FIXME .4 -oplog
