...
Subscription.prototype.remoteOn = function (op, send, store) {
    if (this.pending) { 
        this.pending.push(op); // just in case we have no state
    }
    // TODO unless fwd
    store.push(op);
...
Subscription.prototype.remoteReOn = function (op, send, store) {
    ...
    if (pending===null) {
        // we have the state, ask the storage to make a bundle
        store.push(op);
    } else { // the uplink sent us no state then
        pending.push(op);
        this.askSomeone(send);
    }
};


Subscription.prototype.askSomeone = function (send) {
    var pending = this.pending;
...
Subscription.prototype.remoteBundle = function (op, send, store) {
    ...
    if (this.pending!==null) {
        this.pending.forEach(function(po){
            store.push(po);
        });
        this.pending = null;
    }
...
Subscription.prototype.storageReOn = function (op, send, store) {
    if (!op.value) {
        if (this.uplink) {
            // wait for state arrival to give a meaningful response
            // see remoteReOn()
            return;
        } else {  // we are the root, we have nothing, ask everyone
            this.askSomeone(send);
        }
...
Host.prototype.removePipe = function (spec, pipe) {
    ...
    this.cleanUpLinks(peer_id); // TODO 1Hz
