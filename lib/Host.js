...
module.exports = Syncable.extend(Host, {
    ...
    removeSource: function (spec, error, peer) {
        if (error && peer.url && peer.constructor === Pipe) {
            var uplink_uri = peer.url,
                pipe_opts = peer.opts;

            //reconnect delay for next disconnection
            pipe_opts.reconnectDelay = Math.min(30000, peer.reconnectDelay << 1);

            // schedule a retry
            var self = this;
            setTimeout(function () {
                self.connect(uplink_uri, pipe_opts);
            }, peer.reconnectDelay);
            peer.url = null; //to prevent second reconnection timer
        }

        if (!peer._id) { // skip not handshaken
            return;
        }
