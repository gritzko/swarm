...
Host.prototype.onOp = function (spec, value, source) {
    ...
    source!==o && o.deliver(spec, value, source);
    var dls = this.downlinks[id];
    if (dls) {
        for(var dl=0; dl<dls.length; dl++) {
            var dl_src = this.sources[dl]; // heh
            if (dl_src) {
                if (dl_src!==source) {
...
/** Incoming subscription, essentially a read request for an object.
  ...
  * from a peer connection. `.on` is reciprocated with `.reon` in case
...
Host.prototype.addSource = function hostAddPeer(spec, peer) {
    var old = this.sources[peer.id];
    if (old) {
        old.deliver(this.newEventSpec('off'), '', this);
    }

    this.sources[peer.id] = peer;
...
Host.prototype.onGoodbye = function (spec, nothing, peer) {
    peer.deliver(this.newEventSpec('reoff'), '', this);
    this.removeSource(spec, peer);
};

Host.prototype.onGoodbyeResponse = function hostReOff(spec, nothing, peer) {
    this.removeSource(spec, peer);
};
...
Host.prototype.removeSource = function (spec, peer) {
    if (spec.type() !== 'Host') {
        throw new Error('Host.removeSource(/NoHost)');
    }

    if (this.sources[peer.id] !== peer) {
        console.error('peer unknown', peer.id); //throw new Error
        return;
    }

    delete this.sources[peer.id];

    var relink = [];
    for(var id in this.uplink) {
        if (this.uplink[id] === peer.id) {
            relink.push(id);
            delete this.uplink[id];
        }
    }
    for(var pu_id in this.pending_uplink) {
        if (this.pending_uplink[pu_id] === peer.id) {
            relink.push(pu_id);
...
Host.prototype.getUplink = function (id) {
    var mindist = 4294967295,
        reServer = /^swarm~/, // peers, not clients
...
Host.prototype.disconnect = function (id) {
...
Pipe.prototype.flush = function () {
    ...
    this.pending = {};
    try {
        console.log(this.id,'<',this.host.id, parcel);
...
Pipe.prototype.onStreamClosed = function () {
    ...
    var self = this;
    self.options.reconnect && setTimeout(function (){
        self.host.connect(self.options.uri);
    },self.options.nextReconnectDelay);
};

Pipe.prototype.onStreamError = function () {
};
...
Pipe.prototype.close = function () {
    this.deliver(this.host.newEventSpec('off'),'');
    this.flush();
    this.closed = true;
