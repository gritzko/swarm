...
function Host(id, ms, storage) {
    ...
    this.pending_s = {}; // { /T#id!ver : {value, source_id} }
...
Host.prototype.dispatchRemoteOp = function (op) {
    ...
    // finally, neatly consolidated on/reon/off/reon code   BWAHAHA
    switch (op.name) {
    case 'on': // 
        this.pending_s[op.stamp] = op;
        this.storage.deliver (op);
    break;
    case 'reon':
        // first, ask the storage make a bundle
        this.pending_s[op.stamp] = op;
        this.storage.deliver(op);
        // look for incoming subscriptions pending
        var orig = this.wait_state[op.id];
        delete this.wait_state[op.id];
        while (orig && orig.length) { // respond with .reons
            var stamp = orig.shift();
            var o = this.pending_s[stamp];  // TODO functional, safe
            if (op.value==='') { // the uplink has no state too
                this.send(o.source, o.response('reon', '', this));
            } else { // replay .on to the storage
                this.storage.deliver(o);
            }
        }
    break;
    ...
    case 'bundle':
        this.storage.deliver(op);
        // TODO because we treat .reon as end-handshake, may x bundles
    break;
    case 'state':
...
Host.prototype.dispatchStorageOp = function (op) {
    switch (op.name) {
    case 'reon':
        // HERE WE FINALIZE INCOMING REQUESTS OR POSTPONE THEM

        var pending = this.pending_s[op.stamp];
        if (!pending) {return;}
        // subscribe to the uplink if not yet
        if ( !this.uplink[op.id] && (uplink = this.getUplink(op.id)) ) {
            // .45 FIXME ARBITRARY-LENGTH TIMESTAMPS
            var derived_stamp = op.stamp+'~'+this.id;
            var derived = op.spec.filter('/#').add(derived_stamp, '!');
            if (op.value==='') { // take a shortcut
                this.send( uplink, new Op(derived.add('.on'), '', this) );
            } else { // hint the storage to generate .on
                this.storage.deliver( new Op(derived.add('.preon'), uplink, this) );
            }
            this.uplink[op.id] = uplink; // prevent repetitions
            this.wait_state[op.id] = [];
        }
        // subscribe back to the original source (if we have state)...
        if (op.value || !this.wait_state[op.id]) {
            this.send(pending.source, op);
            // In some cases, simultaneous uplink .on and downlink
            // .reon may lead to two bundles arriving concurrently.
            // Storage deals with that.
            delete this.pending_s[op.stamp]; // the .on is no longer pending
        } else { // ... or wait for state arrival to give a meaningful response
            this.wait_state[op.id].push(op.stamp);
        }
    break;

    case 'on': // a hinted .on from the storage arrives
        var uplink = this.uplink[op.id];
    ...
    case 'bundle': // the client should read ops now => put on the list
        pending = this.pending_s[op.stamp];
        ...
        if (links.indexOf(pending.source)===-1) {
            ...
            // FIXME clean-up on disconnection

            console.warn('@'+this.id, 'repeated subscription', pending.source, op.id);
        }
        this.send(pending.source, op);
        // the initial incoming .on stays pending
    break;
    default: // the storage says it is a new op => relay it
        var links = this.links[op.id] || [];
        for(var i=0; i<links.length; i++) {
            // don't echo to the logix (do we need this?)
            if (links[i]===this.id && op.origin===this.id) { continue; }
...
Host.prototype.registerSyncable = function (spec, obj) {
    ...
    var state = JSON.stringify(obj.toPojo(true));
    this.storage.deliver(new Op(spec.add('.state'), state, this.id));
    ...
    //if (uplink) {    triggered by reon
    //    this.storage.deliver(new Op(ev_spec.add('.preon'), uplink, this.id));
    //}
...
Host.prototype.unregisterSyncable = function (obj) {
    ...
    if (this.storage) {
        this.storage.deliver(new Op(off_spec, '', this.id));
    }
    if (id in this.uplink) {
        var src = this.getSource(this.uplink[id]);
        src && src.deliver(new Op(off_spec, '', this));
        delete this.uplink[id];
    }
};
...
Host.prototype.addSource = function hostAddpipe(spec, pipe) {
    ...
    if (old) {
        old.deliver(new Op(this.newEventSpec('off'), '', this));
    }
...
Host.prototype.dispatchHostHandshake = function (op, pipe) {
    ...
    case 'on':
        ...
        pipe.deliver(new Op(reon, this.clock.ms(), this.id));
    break;
    case 'reon':
        pipe.id = op.id;  // TODO
...
Host.prototype.removeSource = function (spec, pipe) {
    var src = pipe.id;
    if (!this.sources[src]) {return;} // removed already?

    if (this.sources[src] !== pipe) {
        throw new Error('pipe unknown: '+src);
    }
    ...
    for(var id in this.uplink) {
        if (this.uplink[id] === src) {
            relink.push(id);
            delete this.uplink[id];
        }
    }
    for(var pu_id in this.pending_s_uplink) {
        if (this.pending_s_uplink[pu_id] === src) {
            relink.push(pu_id);
            delete this.pending_s_uplink[pu_id];
        }
    }
    while (relink.length) {
...
Host.prototype.connect = function (uri, opts) {
    ...
    client.connect(uri, function(error) {
        ...
        pipe.deliver(new Op(self.newEventSpec('on'), '', self.id));
    });
};
