...
Host.prototype.deliver = function (spec, value, source) {
    ...
    if (!source) { // locally originated
        ...
        switch (op) {
            ...
            if (spec.source()===this.id) { // responds to our subscription
...
Host.prototype.registerSyncable = function (spec, obj) {
    ...
    //this.storage.deliver(ev_spec.add('.on'), obj._version, this); // TODO ver 0
...
Pipe.prototype.deliver = function (spec, value) {
    this.pending.push(new Op(spec, value));
    if (this.asyncFlush) {
