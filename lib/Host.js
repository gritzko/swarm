...
Host.prototype.deliver = function (op, pipe) {
    ...
    if (op.source===this.storage.id) { // own storage
...
Subscription.prototype.remoteReOn = function (op, send, store) {
    ...
    if (pending) { // the uplink sent us no state then
        ...
        var stateful = pending.find(function(o){
            return !!o.value;
        });
        if (stateful) { // ask the downlink that has the state
            pending.splice(pending.indexOf(stateful,1));
            send.push(stateful.response('reon', ''));
...
Subscription.prototype.storageOp = function (op, send, store) {
    ...
    for(var i=0; this.links && i<this.links.length; i++) {
        //if (this.links[i]==='0' && op.origin()===this.id) {
        //    continue;// don't echo to the logix (do we need this?)
        //}
        send.push(op.relay(this.links[i]));
    }
};
