...
Syncable.extend(Syncable, {  // :P
    ...
    deliver: function (spec, value, lstn) {
        ...
        try{
            ...
            if (this._op[call]) {  // FIXME name=>impl table
                if (this.isReplay(spec)) {console.warn('replay',spec); return;} // it happens
                ...
                if (spec.op() !== 'patch') {
                    ...
                    if (!hadState) this.wakeUpWeGotState();
                }
    ...
    emit: function (spec, value, src) {
        ...
        if (ls && ls.length) {
            for (var i = 0; i < ls.length; i++) {
                var l = ls[i];
                if (l && l !== src && l !== ',' && (!is_neutral || l.op === op)) {
    ...
    neutrals: {
        ...
        reon: function (spec, base, repl) {
            ...
            for (var i = 0; i < ls.length && ls[i] !== ','; i++) {
                if (!ls[i]) continue;
                if (ls[i].$pending_ul$ === repl) break; //TODO what about $pending_dl$
                if (repl._host && ls[i].$pending_ul$ === repl._host) break; // direct
            }
            ...
            if (base && (diff = this.diff(base))) {
                repl.deliver(spec.set('.patch'), diff, this); // 2uplink
            }
        ...
        off: function (spec, val, repl) {
            var ls = this._lstn,
                i = ls.indexOf(repl); // fast path
            if (i === -1) {
                //TODO ??? Iterate just listeners to the right of ","
                for (i = 0; i < ls.length; i++) {
                    var l = ls[i];
                    if (!l) continue;
                    if (l.$pending_dl$ && l.$pending_dl$[2] === repl) break;
                    //TODO ??? What about (l.$pending_ul$ === repl) ?
                }
            }
            if (i < ls.length) {
                ls[i] = undefined;
        ...
        reoff: function (spec, val, repl) {
            var ls = this._lstn,
                i = ls.indexOf(repl); // fast path
            if (i === -1) {
                //TODO ??? Iterate just uplink-listeners (to the left of ",")
                for (i = 0; i < ls.length; i++) {
                    var l = ls[i];
                    if (!l) continue;
                    if (l.$pending_ul$ && l.$pending_ul$ === repl) break;
                }
            }
            if (i < ls.length) {
                ls[i] = undefined;
    ...
    wakeUpWeGotState: function () {
        // do deferred diff responses and reciprocal subscriptions
        var pending = [],
            args;
        this._lstn = this._lstn.filter(function (ln) { // FIXME there might be no patch
            if (!ln) return false;
            if (ln.$pending_dl$) {
                pending.push(ln.$pending_dl$);
                return false;
            }
            return true;
        });
        // process deferred on()s
        while (args = pending.pop()) this.deliver(args[0], args[1], args[2]);
    ...
    checkUplink: function () {
        ...
        for (i = 0; i < this._lstn.length && this._lstn[i] != ','; i++) {
            up = this._lstn[i];
            if (!up) continue;
        ...
        for (i = 0; i < new_uplinks.length; i++) {
            ...
            up.deliver(this.newEventSpec('on'),this.version().toString(),this);
        }
    },
...
Swarm.Host = Syncable.extend(Host, {
    ...
    connect: function (stream_or_url, pipe_options) {
        new Pipe(this, stream_or_url, pipe_options).deliver(this.newEventSpec('.on'), '', this);
    },
...
Pipe.prototype.deliver = function pd(spec, val, src) {
    ...
    if (spec.type() === 'Host') {
        if (spec.op() in {off: 1, reoff: 1}) {// grace period
            this.isClient = undefined;
            setTimeout(function close_after_off() { self._id && self.close(); }, 1000);
...
Pipe.prototype.sendBundle = function pS() {
    ...
    try {
        if (Swarm.debug) { console.log('goes to', (this._id || 'undefined'), payload); }
