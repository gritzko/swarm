(function(root, factory) {
    ...
    Spec.Map.prototype.toString = function (trim) {
        ...
        for(var src in map) if (map.hasOwnProperty(src)) {
    ...
    Syncable.extend = function(fn,own) {
        ...
        if (fn.constructor!==Function) {
            ...
            fn = function SomeSyncable(){
                for(var name in fn.defaults) if (fn.defaults.hasOwnProperty(name)) {
        ...
        for(var k in defs) if (defs.hasOwnProperty(k)) {
        ...
        delete own.defaults;
        // add methods
        for (var prop in own) if (own.hasOwnProperty(prop)) {// extend
    ...
    Syncable.extend(Syncable,{  // :P
        ...
        deliver: function (spec,value,lstn) {
            ...
            if (pattern==='/#!.') {
                ...
                var specs = [], sp;
                for (sp in value) if (value.hasOwnProperty(sp)) {
        ...
        version: function () {
            ...
            if (this._oplog) {
                for(var op in this._oplog) if (this._oplog.hasOwnProperty(op)) {
                    map.add(op);
                }
            }
...
//                    this._lstn[1].off(this.spec(), null, this);
        ...
        */
        $_init: function (spec,snapshot,host) {
            if (this._id===spec.version() && !snapshot._oplog) { // new fresh object  TODO nicer
                ...
                for (var sp in this._oplog) if (this._oplog.hasOwnProperty(sp)) {
        ...
        diff: function (base) {
            ...
            if (base && base!='!0') { // diff sync
                ...
                for(spec in this._oplog) if (this._oplog.hasOwnProperty(spec)) {
        ...
        distillLog: function () {
            ...
            for(var s in this._oplog) if (this._oplog.hasOwnProperty(s)) {
                spec = new Spec(s);
                if (spec.method() === 'set') {
                    sets.push(spec);
                }
        ...
        save: function () {
            ...
            for(key in pojo) if (pojo.hasOwnProperty(key)) {
                if (this[key]!==cumul[key]) {// TODO nesteds
                    changes[key] = this[key];
                }
            }
            for(key in cumul) if (cumul.hasOwnProperty(key)) {
                if (!(key in pojo)) {
    ...
    var Set = Swarm.Set = Model.extend('Set', {
        ...
        collection: function () {
            ...
            for(var key in pojo) if (pojo.hasOwnProperty(key)) {
                keys.push(key);
            }
    ...
    Swarm.Host = Syncable.extend(Host,{
        ...
        __on: function (spec,evfilter,peer) {
            ...
            if (evfilter.type() && evfilter.type()!=='Host') { // ??
                ...
                for(var sp in this.objects) if (this.objects.hasOwnProperty(sp)) {
        ...
        availableUplinks: function (spec) {
            ...
            for(var id in this.peers) if (this.peers.hasOwnProperty(id)) {
        ...
        connect: function (host) {
            host.on(this.spec()+'!'+this.version(),'',this); // TODO offset
        },
    ...
    Pipe.prototype.connect = function pc () {
        ...
        if (!self.sink) {
            self.sink = self.opts.sink(); // factory TODO async factory
            if (self.opts.openEvent) {
                self.sink.on(self.opts.openEvent, onOpen);
            } else {
                onOpen();
            }
            function onOpen() {
                if (Swarm.debug) { logger.log('opened'); }
    ...
    Pipe.prototype.parseHandshake = function ph (str) {
        var obj = this.deserialize(str), spec, value, key;
        for (key in obj) if (obj.hasOwnProperty(key)) {
            spec = new Spec(key);
            value = obj[key];
            break; // 8)-
        }
        if (!spec) { throw new Error('not a handshake'); }
        this._id = spec.id();
    ...
    Pipe.prototype.close = function pc () {
        var self = this;
        var logger = self.console || console;
        if (Swarm.debug) { logger.log('closed'); }
...
// milliseconds as an int
    ...
    Pipe.prototype.parseBundle = function pb (msg) {
        var obj = this.deserialize(msg.toString()), keys = [], spec, key;
        for(key in obj) if (obj.hasOwnProperty(key)) {
        ...
        if (!this._id) {// TODO fetch on()
            spec = keys.shift();
            this.parseHandshake(spec,obj[spec]);
            return;
        }
        while (spec = keys.pop()) {
    ...
    Pipe.prototype.sendBundle = function pS () {
        ...
        if (self.sink) try {
            //if (Swarm.debug) {
            logger.log('goes to', (this._id || this.opts.peerName), sendStr);
