(function(root, factory) {
                ...
                if (event) {
                    repl = {
                        ...
                        deliver: function (spec,val,lstn) {
                            spec.method()===event && this.stream.deliver(spec,val,lstn);
                        }
                    };
                }
    ...
    Swarm.Host = Syncable.extend(Host,{
        ...
        getSources: function (spec) {
            ...
            uplinks = uplinks.length ? [ self.sources[uplinks[0].id] ] : [];
            if (!is_serverside && self.storage) { //TODO cache vs storage
                uplinks.push(self.storage);
            }
            return uplinks;
        },
    ...
    Pipe.prototype.connect = function pc (stream) {
        ...
        self.stream.on('close', function onConnectionClosed(reason) {
            ...
            self._id && self.close("stream closed");
            //self.reconnect() ?
        });
    ...
    Pipe.prototype.parseHandshake = function ph (handshake) {
        ...
        if (!spec) throw new Error('handshake has no spec');
        this._id = spec.id();
    ...
    Pipe.prototype.close = function pc (error) {
        ...
        if (error && this.uri) {
            ...
            this.uri = null;
            var reconnectDelay = Math.min(30000, self.reconnectDelay<<1);
            // schedule a retry
            setTimeout(function () {
                new Pipe(self.host,uri,self.opts);
            }, reconnectDelay);
        }
    };
    ...
    Pipe.prototype.sendBundle = function pS () {
        ...
        self.bundle = {};
        if (!self.stream) throw new Error('no stream');

        try {
            if (Swarm.debug) { console.log('goes to', (self._id || 'unintroduced'), sendStr); }
            self.stream.write(sendStr);
            self.lastSendTS = self.time();
        } catch (ex) {
            console.error('stream error: '+ex);
