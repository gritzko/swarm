...
/** All CRDT model classes must extend syncable directly or indirectly. Syncable
  ...
  * by extend() (see 'methods', 'neutrals', 'remotes' sections in class declaration).
  */
...
Syncable.extend(Syncable,{  // :P
    ...
    newEventSpec: function (eventName) { 
        return this.spec().add(this._host.time(),'!').add(eventName,'.');
    },
    ...
    deliver: function (spec,value,lstn) {
        ...
        try{
            ...
            if (typeof(this['$$'+call])==='function') {  // FIXME name=>impl table
    ...
    // Returns a state object that must survive JSON.parse(JSON.stringify(obj))
    diff: function (base) {
    },
    ...
    neutrals: {
        ...
        on: function (spec,filter,repl) {   // WELL  on() is not an op, right?
            ...
            var baseVersion = filter.filter('!'),
                filterByEvent = filter.get('.');

            if (filterByEvent==='init' && this._version) { 
                var diffIfNeeded = baseVersion ? this.diff(baseVersion) : '';
                repl.deliver (spec.set('.init'), diffIfNeeded, this);
                // TODO use once()
            }
            if (filterByEvent) {
                repl = {
    ...
    */
    ...
    diff: function (base) {
        ...
        if (base && base.toString()!=='!0') {
            ...
            for(var s in this._oplog) patch._tail[s] = this._oplog[s];
        }
        return patch;
    },
    ...
    validate: function (spec, val) {
        if (spec.method()!=='set') return ''; // no idea
        for(var key in val)
...
var Set = Swarm.Set = Syncable.extend('Set', {
    ...
    diff: function () {return this._oplog;}, // FIXME
