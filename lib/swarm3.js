    ...
    pojo: function () {
        ...
        for(var key in this) 
            if (Model.reFieldName.test(key) && this.hasOwnProperty(key)) {
                ...
                pojo[key] = def&&def.type ? (val.toJSON&&val.toJSON()) || val.toString() : val;
            }
        return pojo;
    },
    save: function () {
...
var Set = Swarm.Set = Model.extend('Set', {
    add: function (key) {
        // check pattern
        Model.prototype.set.call(this,typeid,'');
    },
    remove: function (key) {
        Model.prototype.set.call(this,typeid,undefined);
    },
    set: function () {}, // block
    get: function (spec) {
        fill();
    ...
    fill: function (key) {
        if (!key) {
            ...
            _host.objects || new this.refType(id) || new Swarm.types[type](id);
        }
    }
});
