...
Syncable.extend(Syncable,{  // :P
    ...
    deliver: function (spec,value,lstn) {
        ...
        try{
            ...
            if (this._method[call]) {  // FIXME name=>impl table
                if (this.isReplay(spec)) return; // it happens
                // invoke the implementation
                this._method[call].call(this, spec, value, lstn); // NOTE: no return value
    ...
    emit: function (spec,value,source) {
        var ls = this._lstn,
            method = spec.method(),
            is_neutral = !!this._neutral[method];
        //this._lstn = []; // cycle protection
        if (ls && ls.length) {
            for (var i = 0; i < ls.length; i++) {
                var l = ls[i];
                if (l && l !== source && l !== ',' && (!is_neutral || l.filterByEvent === method)) {
