...
Syncable.extend = function(fn,own) {
    if (typeof(fn)!=='function') {
        ...
        fn = Syncable.types[id] = function SomeSyncable (id,state,host) {
            ...
            var spec = this.spec();
            if (this._id===version) // TODO prettify
                this._version = version;
            // got state => may init
            state && this.init(spec.toString()+'!'+version+'.init',state,host);
            // register with the host (set up subscriptions, etc)
...
Syncable.extend(Syncable,{
    ...
    __on: function (spec,base,repl) {   // WELL  on() is not an op, right?
        ...
        if (this._lstn[0]) {
            ...
            base && repl.deliver(this.spec(), this.diff(base), this);
            base && repl.reon (this.spec(), this._version || '!0', this);
        } else {
            //var fresh = this._lstn.length===1;
            this._lstn.push( [spec,base,repl] );
            this._lstn.length===2 && this.checkUplink();
        }
...
Swarm.Host = Syncable.extend(Host,{
    ...
    // host.on has some special meaning: it may proxy on() calls
    // to objects; it may create and retrieve objects by their
    // type or id resp.
    ...
    register: function (obj) {
        ...
        this.checkUplink(new Spec(spec));
        return obj;
    },
