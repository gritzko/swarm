        ...
        neutrals: {
            ...
            reon: function (spec,base,repl) {
                ...
                for(var i=0; i<ls.length&&ls[i]!==','; i++)
                    if (ls[i] && ls[i].$pending$===repl) break;
    ...
    var Model = Swarm.Model = Syncable.extend('Model',{
        ...
        */
        init: function (spec,snapshot,host) {
            ...
            if (snapshot) {
                this.apply(snapshot);
                snapshot._oplog && (this._oplog=snapshot._oplog);
            }
            Syncable._pt.init.apply(this,arguments);
        },
