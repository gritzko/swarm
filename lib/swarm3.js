            ...
            if (!this._version && !self.isUplinked()) {
                this._lstn.push({
                    ...
                    deliver: function () {
                        ...
                        self._lstn.splice(i,0,self._lstn.pop());
                    }
                });
...
Swarm.Host = Syncable.extend(Host, {
    ...
    neutrals: {
        ...
        reoff: function hostReOff (spec,ts,host) {
            /*var id = spec.id();
            if (spec.type()!=='Host') throw new Error('TODO');
            if (host===this.sources[id])
                delete this.sources[id]; TODO */
        }
...
Pipe.prototype.connect = function pc (stream) {
    ...
    self.katimer = setInterval(function() {
        var now = self.time(),
            sinceRecv = now - self.lastRecvTS,
            sinceSend = now - self.lastSendTS;
        if (sinceSend > Pipe.TIMEOUT/2) self.sendBundle();
        if (sinceRecv > Pipe.TIMEOUT) self.close("stream timeout");
