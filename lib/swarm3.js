...
Syncable.extend(Syncable,{
    ...
    $_on: function (spec,val,repl) {   // WELL  on() is not an op, right?
        ...
        // if you supply no listener, the object is only
        // guaranteed to exist till the next Swarm.gc() run
        repl && this._lstn.push(repl);
    },
    $_off: function (spec,val,repl) {
        var i = this._lstn.indexOf(repl);
        ...
        if (base||base==='') {
            ...
            function sync () {
                ...
                if (spec.method()==='on' && typeof(repl.reon)==='function')
                    repl.reon (self.spec(), self.version(), self);
            };
            this._version ? sync() : this.once(sync);
        } else { // plain listener
            // add it as a filtered set() listener
        }
        Syncable.prototype.$_on.apply(this,arguments);
    },
...
Swarm.Host = Syncable.extend(Host,{
    ...
    $_off: function (spec,nothing,peer) {
        ...
        for(var sp in this.objects) 
            if (this.uplink(sp)===peer)
                this.relink(this.objects[sp]);
