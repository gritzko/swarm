(function(root, factory) {
    ...
    Syncable.extend = function(fn,own) {
        ...
        // "Locals" don't change the state
        for (var name in own.neutrals||{}) {
            if (!Syncable.reMethodName.test(name)) continue;
            fnproto['__'+name] = own.neutrals[name];
            fnproto[name] = wrapCall(name,'__'+name);
        }
    ...
    // PROPOSAL ***
    // Hide specifiers, simplify external signatures, remove ambiguity
    // * direct calls have no spec
    // * have spec => call deliver(), directly to the impl
    // * deliver() does log, listeners
    // * trigger('event',{param:val}) is a wrapper for _emit(spec...)
    // * _methods
    // * methods : {}, calls : {}, invokables: {}, closeds : {}
    ...
    Syncable.extend(Syncable,{  // :P
        ...
        deliver: function (spec,value,lstn) {
            ...
            if (pattern==='/#') { // unbundle if needed
                var specs = [], sp;
                for (sp in value) Spec.pattern(sp)==='!.' && specs.push(sp);
                specs.sort().reverse();
                while (sp = specs.pop()) this.deliver(''+spec+sp,value[sp],lstn);
                return sp;
            }
            spec = Spec.as(spec);
            var opver = spec.version();
            ...
            try{
                var methodImpl = '$$'+spec.method();
                if (typeof(this[methodImpl])==='function') {
                    this[methodImpl](spec,value,lstn); // NOTE: no return value
                } else if (spec.method()==='init') {
                    if (!this._version)
                        this.init(spec,value,lstn);
                    return; // TODO resync-short log (make a test)
                } else {
    ...
    //                    this._lstn[1].off(this.spec(), null, this);
        ...
        checkUplink: function () {
            ...
            var evspec = this.newEventSpec('on');
            closest.deliver(evspec,this.version(),this);
    ...
    Swarm.Host = Syncable.extend(Host,{
        ...
        addPeer: function hostAddPeer (spec,peer) {
        ...
        neutrals: {
            ...
            on: function hostOn (spec,evfilter,peer) {
                if (!evfilter)
                    return this.addPeer(spec,peer);
