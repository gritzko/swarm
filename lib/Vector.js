...
module.exports = Syncable.extend('Vector', {
    ...
    ops: {  // operations is our assembly language
        ...
        in: function (spec, value, src) {
            ...
            var type = value.tok('/') || this.objectType; // TODO dirty
            if (!type) { throw new Error('object type not specified'); }
    ...
    inSubtreeOf: function (nodeId, rootId) {
        for(var id=nodeId, p=id; id>rootId; p=id, id=this.getParentOf(id));
        return id===rootId && p;
    },

    isDescendantOf: function (nodeId, rootId) {
        for(var i=id; i>parentId; i=this.getParentOf(i));
        return i===parentId;
    },

    skipSubtree: function (iter, root) {
        var root = root || iter.token();
        do {
            iter.next();
    ...
    /*indexOf: function (obj, startAt) {
        if (_id in obj) { return this.objects.indexOf(obj); }
        if (obj.constructor===String) {
            for(var i=0; i<this.objects.length; i++) {
                if (this.objects[i]._id===obj) {
                    return i;
                }
            }
        }
    ...
    // event relay
    //
    deliverObjectEvent: function (spec,val,src) {
    },

    onObjectEvent: function (spec,val,src) {
    },

    offObjectEvent: function (spec,val,src) {
    }
