...
module.exports = Syncable.extend('Vector', {
    ...
    ops: {  // operations is our assembly language
        ...
        rm: function (spec, value, src) {
            ...
            // (!) vector may contain the same object more than once
            // check we removed the last object entry
            if (this.every(function (entry) { return entry !== obj; })) {
                // and if so  unsubscribe proxy from the object
                obj.off(this._proxy);
            }
        }
    ...
    reactions: {
        ...
        init: function fillAll (spec,val,src) { // TODO: state init tests
