...
FileStorage.prototype.appendToLog = function appendToLog (tidoid,verop2val) {

    var tail = this.tails[tidoid];
    if (!tail) this.tails[tidoid] = tail = {};
    ...
    this.dirtyQueue.push(tidoid.toString());
    // serialize the op as JSON
    var o = {}, self=this;
    o[tidoid] = verop2val;  // TODO annoying
...
FileStorage.prototype.pullState = function pullState () {
    var spec;
    ...
    this.host.deliver(tidoid+'.on','.init!0',this);
};

FileStorage.prototype.patch = function fsPatch (spec,patch,src) {
    ...
    var fn = this.stateFileName(ti), dir = path.dirname(fn);
    ...
    fs.writeFile(fn, save, function onSave(err){
        if (err) {
            ...
            self.pulling = 0;
            self.pullState(); // may request next object
        }
    });

};

FileStorage.prototype.op = function appendOp (spec,val,src) {
    var ti = spec.filter('/#'), vo = spec.filter('!.'), o={};
    o[vo] = val;
...
FileStorage.prototype.on = function (spec,base,replica) {
    ...
    function stateVersionVector (state) {
        var map = new Spec.Map(state._version+(state._vector||''));
    ...
    fs.readFile(statefn,function onRead(err,data){
        state = err ? {_version:'!0'} : JSON.parse(data.toString());
        ...
        var vv = stateVersionVector(state);

        replica.deliver (ti.add(spec.version(),'!').add('.patch'), state, self);

        replica.deliver (ti.add(spec.version(),'!').add('.reon'), vv, self);
    });
...
FileStorage.prototype.loadTails = function loadTails () {
    ...
    for(var i=0; i<logs.length; i++) {
        ...
        var count = this.parseLogFileName(log);
        ...
        for(var j=0; j<arr.length; j++) {
            ...
            for(var tidoid in block) {
                var tail = this.tails[tidoid], ops = block[tidoid];
                if (!tail) {
