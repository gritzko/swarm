...
Spec.parseToken = function (token) {
    Spec.reQTokExt.lastIndex = 0;
    var m = Spec.reQTokExt.exec(token.toString());
    return m ? {
        quant: m[1], 
        body: m[2],
        bare: m[3],
...
Spec.prototype.parseToken = function (quant) {
    return Spec.parseToken(this.tok(quant));
};
...
Spec.Map.prototype.add = function (versionVector) {
    var filtered = new Spec(versionVector,'!').filter('!').toString();
    if (filtered==='') {return;}
    var toks = filtered.match(Spec.reQTokExt);
    while (toks.length) {
        var tok = Spec.parseToken(toks.pop());
