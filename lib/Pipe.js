...
function Pipe (host, stream, opts) {
    ...
    if (typeof(stream.write) !== 'function') { // TODO nicer
        ...
        var fn = swarmOptions.streams[proto];
...
Pipe.prototype.connect = function pc (stream) {
    ...
    self.stream.on('data', function onMsg(data) {
        data = data.toString();
        if (swarmOptions.debug) console.log('@'+(self.host&&self.host._id),'<'+(self._id||'???'), data);
    ...
    self.stream.on('close', function onConnectionClosed(reason) {
        if (swarmOptions.debug) console.log('sink closed');
...
Pipe.prototype.close = function pc (error) {
    if (swarmOptions.debug) console.log('pipe closed', this._id, error);
...
Pipe.prototype.sendBundle = function pS() {
    ...
    try {
        if (swarmOptions.debug) console.log('@'+this.host._id,'>'+(this._id||'???'), payload);
        this.stream.write(payload);
        this.lastSendTS = this.time();
    } catch (ex) {
        console.error('stream error on write: ' + ex, ex.stack);
