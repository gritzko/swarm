...
Pipe.prototype.keepAliveFn = function () {
    ...
    // send keep-alive messages only when pipe already handshaken
...
Pipe.prototype.close = function pc(error) {
    ...
    if (this.host) {
        if (this.isOnSent !== undefined) {
            // emulate normal off
            var offspec = this.host.newEventSpec(this.isOnSent ? 'reoff' : 'off');
            this.host.deliver(offspec, error || '', this);
        }
