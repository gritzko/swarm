...
Pipe.prototype.connect = function pc(stream) {
    ...
    self.stream.on('data', function onMsg(data) {
        data = data.toString();
        env.trace && env.log(this, dotIn, data, this);
...
Pipe.prototype.close = function pc(error) {
    env.log(this, dotClose, error ? 'error: ' + error : 'correct', this);
...
Pipe.prototype.sendBundle = function pS() {
    ...
    try {
        env.trace && env.log(this, dotOut, payload, this._host);
...
Pipe.prototype.toString = function () {
    return 'Pipe@' + (this._host && this._host._id || '?') + '@' + (this._id || '?');
};
