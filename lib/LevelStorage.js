...
function LevelStorage (filename,callback) {
    Storage.call(this);
    ...
    var self = this;
    self.db = level(this.filename,function(err,db){
        self.db = db;
...
LevelStorage.prototype.isRoot = true;
...
LevelStorage.prototype.readState = function (ti, callback) {
    ...
    this.db.get(ti, function(err,value){
        value = value ? JSON.parse(value) : {_version: '!0'}; //?
        console.log('<STATE',self._host._id,value);
...
LevelStorage.prototype.readOps = function (ti, callback) {
    ...
    var stream = this.db.createReadStream({
        ...
        console.log('<TAIL',self._host._id,tail);
