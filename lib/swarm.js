...
SpecValEmitter._p.emit = function (spec,val,src) {
    ...
    for (var j=0; j<listeners.length; j++) {
        ...
        try{ 
            ...
            console.error('error in the listener',ex,ex.stack);
        }
    }
};
...
Peer.prototype.addPeer = function (peer) {
    ...
    for(var object in self._lstn) {
        ...
        if (newup===peer) {
            ...
            newup.on (obj?obj.spec()+obj.base():object, this);
        }
    }
};
...
Peer.extend = function(func,name6,defaults) {
    if (name6.constructor===Object && !defaults) {
        defaults = name6;
        name6 = '';
    }
...
var SwarmObjectProto = {
    ...
    set: function the_set (spec,val,srcref) {
        ...
        for(var sp in val) {
            var value = val[sp];
            if (!Spec.is(sp)) {
                var fid = Spec.wireNames[sp];
                if (!fid)
                    throw new Error('unknown literal', field);
                if (val[fid])
                    continue;
                else
                    val[fid] = value;
            ...
            sp = new Spec(sp,spec);
            if (!sp.version) {
                spec.version = sp.version =
                    this._host ? this._host.createId('!') : new ID('!');
                spec.cache = sp.cache = '';
                srcref = this; // well...
            }
    ...
    base: function the_base () {
        var m = [], max = {}, ret = [];
        ID.re_id_g.lastIndex = 0;
        while (m=ID.re_id_g.exec(this._vmap)) {
            if (m[1]!=='!') continue;
            var src = m[4]+m[5];
            if (m[0]>(max[src]||''))
                max[src] = '$'+m[2]+m[3]+m[4]+m[5];
        }
        for(var k in max)
            ret.push(max[k]);
    ...
    diff: function the_diff (spec,diff) {
        ...
        while (field=versions.pop()) {
            ...
            if ( version > (spec.base[source]||'').replace('$','!') ) { // TODO prettify
                diff = diff || {};
                diff[field+version] = this[ this.isSet ? field : Spec.wireNames[field] ];
            }
        }
        return diff;
    }
...
Pipe.prototype.parseHandshake = function(spec,specme) {
...
Plumber.prototype.scheduleReconnect = function (uri) {
    var self = this;
    var delay = Math.max(10,++self.reconnCount[uri])<<6;
    setTimeout(function(){
        self.connect(uri);
    },delay);
};
...
Plumber.prototype.connect = function (uri) {
    ...
    var vocab = Plumber.schemes[uri.scheme].eventNames || {};
    socket.on(vocab.open||'open',function(err){
        if (err)
            return self.scheduleReconnect(uri);

        var pipe = new Pipe(gotId,socket,self.host,{});

        //self.host.addPeer(pipe);
        self.reconnCount[uri] = 0;
        socket.on(vocab.close||'close',function(){
            //self.host.removePeer(pipe);
            self.scheduleReconnect(uri);
            pipe = null;
        });
        /*socket.on(vocab.error||'error',function(err){
            console.error('sockeet error: '+err);
        });*/
    });
};
