...
Peer.prototype.addPeer = function (peer) {
    ...
    self.peers[pid] = peer;
    if (!peer.hashes)
        peer.hashes = Peer.hashRing(pid);
    // /TODO REFAC
    
    peer.on(Spec.METHOD_ON,function(spec,val){
        this.on(spec,val,peer);
    });
    peer.on(Spec.METHOD_OFF,function(spec,val){
...
Peer.idDistance = function (hash,peer) {
    hash = ID.as(hash);
    peer = ID.as(peer);
    return hash.ts ^ peer.ts; // FIXME
};

Peer.distance = Peer.prototype.distance = function (idhash,peer) {
    peer = peer || this;
    if (!peer._hashes)
    ...
    for(var i=0; i<hashes.length; i++)
        if ( (hashes[i]^idhash) < minDist )
            minDist = hashes[i]^idhash;
    return minDist;
};

Peer.prototype.findUplink = function (obj,except) {
    var nopid = except ? except.id || except : '';
    var minDist = Peer.idDistance(obj,this),
        minPeer = this.storage;
    var hash = ID.as(obj._id).hash();
    for(var pid in this.peers) {
        if (pid===nopid) continue;
        var peer = this.peers[pid];
        var dist = Peer.idDistance(hash,peer);
        if (minDist>=dist) 
