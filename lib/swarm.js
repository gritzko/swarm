...
SpecValEmitter._p.on = function (spec,fn) {
    ...
    if (SpecValEmitter._debug)
        console.debug('\ton',(this._id&&this._id.toString())||this.constructor.name,
                '>>',(fn._id&&fn._id.toString())||fn.constructor.name,spec.toString());
...
SpecValEmitter._p.off = function (spec,fn) {
    if (SpecValEmitter._debug)
        console.debug('\toff',(this._id&&this._id.toString())||this.constructor.name,
                '>X',(fn._id&&fn._id.toString())||fn.constructor.name,spec.toString());
    if (this._lstn.constructor===Array) {
...
Peer.prototype.findObject = function (spec) {
    var oid = Spec.as(spec).oid.toString();
...
Peer.prototype.createObject = function(spec,prefab) {
    ...
    if (needUpOn) {
        ...
        up.on(oid+'$000000',this); // no base; we need a complete state boot
    }
    return obj;
};
...
Peer.prototype.on = function (spec,listener) {
    ...
    if (ln && ln._id && ln._id in this.peers) {
        ...
        if (Peer.distance(spec,ln) > Peer.distance(spec,this))  // FIXME  dist==dist
            ln.on(obj.spec(),this);
    }
