...
/***/
function Spec (spec, scope, quant) {
    this.type =
        this.id = 
        this.field = 
        this.key = 
    ...
    if (!m) {
        if (scope && scope._literals && spec in scope._literals)
            spec = scope._literals[spec];
        else
...
Peer.prototype.createObject = function(spec,prefab) {
    ...
    var fn = Peer.prototypes[spec.type.toString32()], object = spec.id;
    if (!fn)
        ...
        if (!spec.field) { 
            for (var s in val)
                if (s.charAt(0)!='_') {
                    ...
                    this.set(spec,val[s],srcref);  // TODO unite with the Diff logic ^^^
                }
            return; // BAD
        }
