 ...
 *  /coll#object.key!ver  val
 *  /coll#set.id!ver  /coll1/coll2  -- optional as object is glob-unique
_ *  /coll#doc.pid!ver  :pos string
 *
...
/***/
function Spec (spec, scope, quant, vocab) {
    this.type =
        this.id = 
        this.field = 
        this.key = 
    ...
    if (!m) {
        if (vocab && (spec in vocab) )
            spec = vocab[spec];
        else
            spec = ID.parse32(quant,spec).toString();
    }
...
Spec.order = ['type','id','field','key','method','version','base','peer'];
Spec.quants = '/#.,\'!$*';
...
SpecValEmitter._p.on = function (spec,fn) {
    ...
    var key = spec.id || spec.field || spec.method; // one parameter only
...
Peer.prototype.addPeer = function (peer) {
    ...
    for(var object in self._lstn) {
        if (object.charAt(0)!=='#') continue;
        var newup = this.findUplink(object), oldup;
        if (newup===peer) {
            if (oldup = this.findUplink(object,newup))
                oldup.off(object,this);
            var obj = this.findObject(object);
            newup.on (obj?obj.spec():object, this);
        }
    }
};
...
Peer.prototype.removePeer = function (peer) {
    ...
    for(var object in this._lstn) {
        ...
        var oldup = this.findUplink(object);
        if (oldup===peer) {
            var newup = this.findUplink(object,oldup); // TODO merge
            var obj = this.findObject(object);
            newup.on(obj?obj.spec():object,this);
            oldup.off(object,this);
        }
...
Peer.prototypes = {};
Peer.re_varname = /^[a-z][a-z0-9]*([A-Z][a-z0-9]*)*$/; // camel case

Peer.makeNameVocabulary = function (sample) {
    var vocab = {};
    for (var name in sample) {
        if (name.charAt(0)==='_') continue;
        if (!sample.hasOwnProperty(name)) continue;
        if (!Peer.re_varname.test(name)) continue;
        var code = name;
        while (code.length<6)
            code = '='+code;
        if (code.length>6)
        ...
        if (code in vocab) throw new Error('well...');
        code = '.'+code;
        vocab[code] = name;
        vocab[name] = code;
    }
...
Peer.extend = function(func,vocab) {
    // may introspect an empty object
    vocab = vocab || Peer.makeNameVocabulary(new func());
    for(name in vocab) vocab[vocab[name]] = name;
    var fname = func.name;
    var tid = new Spec(vocab[fname]||fname,'','/',vocab);
    Peer.prototypes[fname] = Peer.prototypes[tid] = func;
    var proto = func.prototype;
    proto._type = tid.toString();
    proto._defQuant = '.';
    proto._vocab = vocab;
    // extend the prototype
    for (var name in vocab) {
        var fid = vocab[name];
        if (fid.charAt(0)!=='.') continue;
...
var SwarmObjectProto = {
    ...
    set: function the_set (spec,val,srcref) {
        ...
        spec = new Spec(spec,this._type+this._id,'.',this._vocab);
        ...
        for(var field in val) {
            var value = val[field];
            if (!Spec.is(field) || field.charAt(0)!=='.') {
                delete val[field];
                field = new Spec(field,'','.',this._vocab).toString();
                val[field] = value;
            }
            var hasVersion = Spec.getPair(this._vmap,field);
            if ((hasVersion||'') < spec.version) {
                var fname = this._vocab[field] || field;//.toString32();
                this[fname] = val;
                this._vmap = Spec.setPair(this._vmap,field,spec.version);
                if (this._lstn.constructor===Object)
                    this.emit(new Spec(field,spec),value,srcref);
                else 
                    doEmit = true;
            } else
                console.warn('too old');
        }
        if (doEmit && this._lstn.constructor===Array)
            this.emit(spec,val,srcref);
    },

    spec: function () { return this._type + this._id + this.base(); },
    ...
    diff: function the_diff (spec,diff) {
        ...
        while (field=versions.pop()) {
            ...
            if ( version > (spec.base[source]||'').replace('$','!') ) { // TODO prettify
                ...
                diff[this._type+this._id+field+version] = this[this._unwire[field]];
            }
        }
        return diff;
    }
...
/*   Pain points
 *   * base in Spec
 *   * vocabulary-based base64 conversion
 *   * diff/field-val-map/bundle/spec-val/key-val morph
