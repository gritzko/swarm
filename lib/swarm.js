...
Peer.prototype.createOid = function (proto) {
    ...
    return '/'+proto+this.createVid().replace('$','#');
};
...
Peer.createObject = function (oid) {
    var m = oid.match(Peer.re_object_id);
    if (!m)
...
Peer.JsonSeDe.prototype.on = function (obj) {
    this.send(obj._id,new Peer.vmax(obj._vmap));
};
...
Peer.JsonSeDe.prototype.onMessage = function (mesg) {
    ...
    for(var oid in msg) {
        var op = msg[oid];
        var obj = this.peer.objects[oid];
        if (op===null) {    // off
            if (obj)
                obj.off(this);
        } else if (op.constructor===String) {  // on, vmax
            if (obj) {
                var diff = obj.diff(op);
                if (diff) {
                    var send = {};
                    send[oid] = diff;
                    this.pipe.send((send)); // apply back
                }
            }
