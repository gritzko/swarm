...
Spec2.as = function (spec) {
    return spec.constructor===Spec2 ?
        spec : new Spec2(spec.toString()||'');
};

Spec2.is = function (str) {
    return str.constructor===Spec2 || ID.re_id_g.test(str.toString());
}
...
Spec2.prototype.parse = function (quant) {
...
Peer.extend = function(func,name) {
    ...
    for (var f in sample) {
        ...
        proto['set'+capname] = function (val) {
            this.set(f,val);
        }
    ...
    set: function the_set (spec,val,srcref) {
        if ( spec==='' || (spec.constructor===Spec2 && spec.field==='') ) {
            ...
            if ((hasVersion||'') < spec.version) {
                ...
                this[fname] = val;
                this._vmap = Spec2.setPair(this._vmap,spec.field,version);
...
Peer.JsonSeDe.prototype.set = function (spec,val) {
    if (spec===this.echo)
        return;
    if (val._id)
        this.send(val);
    ...
    for(var sp in msg) {
        ...
        if (/^\/\#\!*$/.test(pattern)) { // on / off
            ...
            if (obj) {
                ...
                console.error('set to a closed object');
        } else {
