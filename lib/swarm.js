...
function ID (str,src,ssn) {
    if (str.length===7) {
        ...
        this.q = str;
        this.ts = ((new Date().getTime()/1000)|0) - SWARM_EPOCH;
...
ID.parse32 = function (quant,base32) {
    ...
    if (!m) throw new Error('malformed base32 RFC4648 "9-9" id',base32);
...
function Spec2 (spec) {
    var m=str!==''?str.match(ID.re_id_g):[], tok;
    if (!m || str.length!=m.length*7)
...
Spec2.prototype.toString = function () {
    return this.cache ||
        (this.cache = this.type.toString()+this.oid.toString()+this.field.toString()+
        this.key.toString()+this.method.toString()+this.revision.toString()+this.base.toString());
};
...
function Peer (src,ssn) {
    ...
    this.src = Spec._int2uni(src);
    ssn = ssn || '0';
    if (typeof(ssn)==='number')
        ssn = Spec._int2uni(ssn).charAt(1);
    this.ssn = ssn;
    this.id = this.createId('*',null,this.src);
    this.lastTs = this.lastSeq = 0;
    this.peers = {};
    this.objects = {};
...
Peer.prototype.createId = function (quant,ts,src,ssn,seq) {
    quant = quant || '!';
    if (!ts)
    ...
    seq = seq || Peer.lastSeq++;
    if (typeof(seq)==='number')
        seq = Spec._int2uni(seq).charAt(1);
    var id =  quant + ts + seq + ssn + src;
    if (!Spec.re_id_uni.test(id))
        throw new Error('malf id',id);
    return id;
};
